...
// Fixed fused-production geometry.
constexpr int kExchangePipelineDepth = 3;
constexpr int kExchangeRowsPerWarp = 24;
...
// lane in the warp's value pipeline.
...
// All builds still emit SM75, whose statically declared per-block shared
// memory is limited to 48 KiB.
constexpr size_t kExchangeRawAndResolvedSharedBytes =
    sizeof(ExchangeRawMetadata) +
    sizeof(ExchangeMetadata) * kExchangeInputsPerBlock;
constexpr size_t kExchangePipelineSharedBytes =
    kExchangeWarpsPerBlock * kExchangePipelineDepth * kExchangeDirections *
    kExchangeTileBytes;
static_assert(kExchangeRawAndResolvedSharedBytes +
                      kExchangePipelineSharedBytes <=
                  48 * 1024,
              "exchange kernel exceeds SM75 static shared-memory limit");
...
// Keep only the fields used by one side of the value pipeline live in
// registers. In particular, do not snapshot the reference-counter fields.
struct ExchangeIssueMetadata {
  int64_t cache_to_storage_src;
  int64_t storage_to_cache_src;
  int64_t value_dim;
  uint32_t flags;
...
__forceinline__ __device__ ExchangeIssueMetadata
exchange_load_issue_metadata(const ExchangeMetadata &metadata) {
  return ExchangeIssueMetadata{metadata.cache_to_storage_src,
                               metadata.storage_to_cache_src,
                               metadata.value_dim, metadata.flags};
}

__forceinline__ __device__ ExchangeStoreMetadata
exchange_load_store_metadata(const ExchangeMetadata &metadata) {
  return ExchangeStoreMetadata{metadata.cache_to_storage_dst,
...
__forceinline__ __device__ void exchange_issue_stage(
    const ExchangeIssueMetadata &metadata, int64_t tile_offset, ValueT *smem) {
...
template <typename ValueT>
__forceinline__ __device__ void exchange_store_stage_pair(
    const ExchangeStoreMetadata &metadata, int64_t tile_offset,
    const ValueT *cache_to_storage_smem,
    const ValueT *storage_to_cache_smem) {
  constexpr int kValuesPerThread = kExchangeBytesPerThread / sizeof(ValueT);
  constexpr int kExchangeTileValues = kExchangeTileBytes / sizeof(ValueT);
  constexpr uint32_t kBothDirections =
      kExchangeMetadataCacheToStorage |
      kExchangeMetadataStorageToCache;
  const bool both_active =
      (metadata.flags & kBothDirections) == kBothDirections;
  const bool full_tile =
      metadata.value_dim >= kExchangeTileValues &&
      tile_offset <= metadata.value_dim - kExchangeTileValues;

  if (both_active && full_tile) {
    const int lane = threadIdx.x & (kExchangeWarpSize - 1);
    const int64_t offset =
        tile_offset + static_cast<int64_t>(lane) * kValuesPerThread;
    ValueT *cache_to_storage_dst =
        reinterpret_cast<ValueT *>(metadata.cache_to_storage_dst) + offset;
    ValueT *storage_to_cache_dst =
        reinterpret_cast<ValueT *>(metadata.storage_to_cache_dst) + offset;
    if (exchange_address_aligned(cache_to_storage_dst, 16) &&
        exchange_address_aligned(storage_to_cache_dst, 16)) {
      const ValueT *cache_to_storage_thread_smem =
          cache_to_storage_smem + lane * kValuesPerThread;
      const ValueT *storage_to_cache_thread_smem =
          storage_to_cache_smem + lane * kValuesPerThread;
      const ExchangeBytes16 cache_to_storage_value =
          *reinterpret_cast<const ExchangeBytes16 *>(
              cache_to_storage_thread_smem);
      const ExchangeBytes16 storage_to_cache_value =
          *reinterpret_cast<const ExchangeBytes16 *>(
              storage_to_cache_thread_smem);
      // Force both shared-memory values to be materialized before either
      // mapped-global store. This gives the first LDS an independent LDS to
      // cover part of its short-scoreboard latency.
      asm volatile(""
                   :
                   : "r"(cache_to_storage_value.value[0]),
                     "r"(cache_to_storage_value.value[1]),
                     "r"(cache_to_storage_value.value[2]),
                     "r"(cache_to_storage_value.value[3]),
                     "r"(storage_to_cache_value.value[0]),
                     "r"(storage_to_cache_value.value[1]),
                     "r"(storage_to_cache_value.value[2]),
                     "r"(storage_to_cache_value.value[3])
                   : "memory");
      *reinterpret_cast<ExchangeBytes16 *>(cache_to_storage_dst) =
          cache_to_storage_value;
      *reinterpret_cast<ExchangeBytes16 *>(storage_to_cache_dst) =
          storage_to_cache_value;
      return;
    }
  }

  exchange_store_tile(
      reinterpret_cast<ValueT *>(metadata.cache_to_storage_dst),
      metadata.value_dim, tile_offset, cache_to_storage_smem,
      (metadata.flags & kExchangeMetadataCacheToStorage) != 0);
  exchange_store_tile(
      reinterpret_cast<ValueT *>(metadata.storage_to_cache_dst),
      metadata.value_dim, tile_offset, storage_to_cache_smem,
      (metadata.flags & kExchangeMetadataStorageToCache) != 0);
}
...
__forceinline__ __device__ void exchange_resolve_metadata(
  ...
  *metadata = ExchangeMetadata{};
  ...
  // Final cache Busy results trap in find_or_insert. A negative cache row can
  // still remain when a failed backing insertion rolls back a provisional
  // cache eviction. A storage hit for that position stays acquired and is
  // consumed directly by the mixed forward/update path.
  const bool storage_to_cache =
      raw.storage_founds[local_input] && cache_row >= 0;
  const bool cache_to_storage =
      raw.evicted_mask[local_input] && cache_row >= 0;
  ...
  if (storage_to_cache) {
    ...
    if (storage_row >= 0 && lookup_slot >= 0 &&
      ...
      metadata->flags |= kExchangeMetadataStorageToCache;
      metadata->storage_to_cache_src =
  ...
  if (cache_to_storage) {
    ...
    if (storage_row >= 0 && insert_slot >= 0 &&
        insert_slot < table_capacity) {
      metadata->flags |= kExchangeMetadataCacheToStorage;
...
__global__ void exchange_cache_storage_values_kernel(
    ...
    int64_t *__restrict__ storage_src_rows,
    int64_t *__restrict__ storage_lookup_slots,
...
#pragma unroll 1
    ...
    // Convert the lookup metadata in place to direct-storage fallback
    // metadata. Normal cache materializations become -1 and release their
    // lookup reference below. A storage hit with no cache row keeps both its
    // physical row and hash slot so forward/backward can use the backing row
    // directly and release that reference after the optimizer update.
    for (int local_input = threadIdx.x;
         local_input < kExchangeInputsPerBlock; local_input += blockDim.x) {
      const int64_t input_id = block_input + local_input;
      if (input_id < batch) {
        const bool direct = raw_metadata.cache_rows[local_input] < 0 &&
                            raw_metadata.storage_founds[local_input] &&
                            raw_metadata.storage_src_rows[local_input] >= 0 &&
                            raw_metadata.storage_lookup_slots[local_input] >= 0;
        storage_src_rows[input_id] =
            direct ? raw_metadata.storage_src_rows[local_input] : -1;
        storage_lookup_slots[input_id] =
            direct ? raw_metadata.storage_lookup_slots[local_input] : -1;
      }
    }
    ...
    // Compact rows with value traffic, then flatten each row's tiles into one
    // continuous stream. Each group consumes the cache snapshot before
    // storage-to-cache overwrites the same physical row.
...
#pragma unroll 1
    for (int row = 0; row < kExchangeRowsPerWarp; ++row) {
      ...
      if ((item.flags &
        ...
        total_stages +=
            (item.value_dim + kExchangeTileValues - 1) /
            kExchangeTileValues;
      }
    }
    ...
    if (lane < kExchangeRowsPerWarp) {
      const ExchangeMetadata &item =
          metadata[warp + lane * kExchangeWarpsPerBlock];
      lane_active =
          (item.flags &
           (kExchangeMetadataStorageToCache |
            kExchangeMetadataCacheToStorage)) != 0;
    ...
    const uint32_t active_mask =
        __ballot_sync(0xffffffffU, lane_active);

    if (total_stages != 0) {
      // The ballot already encodes compact rows in ascending lane order.
      // Producer and consumer independently replay that same flattened
      // row/tile stream, avoiding shared-memory row and stage-tag rings.
      uint32_t producer_rows = active_mask;
      int producer_original_row =
          __ffs(static_cast<int>(producer_rows)) - 1;
      int64_t producer_tile_offset = 0;
      ExchangeIssueMetadata producer_metadata = exchange_load_issue_metadata(
          metadata[warp +
                   producer_original_row * kExchangeWarpsPerBlock]);

      // Always prime depth - 1 logical groups. Once the compacted stream is
      // exhausted, converged empty groups keep the per-lane cp.async group
      // sequence identical and make the wait distance a constant immediate.
#pragma unroll
      for (int stage = 0; stage < kExchangePipelineDepth - 1; ++stage) {
        if (stage < total_stages) {
          exchange_issue_stage<ValueT, kExchangeCacheToStorage>(
              producer_metadata, producer_tile_offset,
              &smem[warp][stage][kExchangeCacheToStorage][0]);
          exchange_issue_stage<ValueT, kExchangeStorageToCache>(
              producer_metadata, producer_tile_offset,
              &smem[warp][stage][kExchangeStorageToCache][0]);

          producer_tile_offset += kExchangeTileValues;
          if (producer_tile_offset >= producer_metadata.value_dim) {
            producer_rows &= producer_rows - 1U;
            producer_tile_offset = 0;
            if (producer_rows != 0) {
              producer_original_row =
                  __ffs(static_cast<int>(producer_rows)) - 1;
              producer_metadata = exchange_load_issue_metadata(
                  metadata[warp + producer_original_row *
                                      kExchangeWarpsPerBlock]);
            }
          }
        } else {
          __syncwarp();
      ...
      uint32_t consumer_rows = active_mask;
      int consumer_original_row =
          __ffs(static_cast<int>(consumer_rows)) - 1;
      int64_t consumer_tile_offset = 0;
      ExchangeStoreMetadata consumer_metadata = exchange_load_store_metadata(
          metadata[warp +
                   consumer_original_row * kExchangeWarpsPerBlock]);
      int head_slot = 0;
      int tail_slot = kExchangePipelineDepth - 1;
      for (int64_t head = 0; head < total_stages; ++head) {
        // Metadata is immutable after resolution. Keeping the current row's
        // slim store view live here moves its shared loads before the wait and
        // reuses it for every tile in that row.
        // depth - 2 is a compile-time immediate. With depth - 1 pending
        // groups this waits for exactly the oldest flattened stage.
        __pipeline_wait_prior(kExchangePipelineDepth - 2);
        __syncwarp();

        const int64_t tail = head + kExchangePipelineDepth - 1;
        if (tail < total_stages) {
          exchange_issue_stage<ValueT, kExchangeCacheToStorage>(
              producer_metadata, producer_tile_offset,
        ...
        exchange_store_stage_pair<ValueT>(
            consumer_metadata, consumer_tile_offset,
            &smem[warp][head_slot][kExchangeCacheToStorage][0],
            &smem[warp][head_slot][kExchangeStorageToCache][0]);
        // Finish every lane's shared-memory read before this slot can be
        // recycled by the next producer iteration.
        __syncwarp();

        consumer_tile_offset += kExchangeTileValues;
        if (consumer_tile_offset >= consumer_metadata.value_dim) {
          consumer_rows &= consumer_rows - 1U;
          consumer_tile_offset = 0;
          if (consumer_rows != 0) {
            consumer_original_row =
                __ffs(static_cast<int>(consumer_rows)) - 1;
            consumer_metadata = exchange_load_store_metadata(
                metadata[warp + consumer_original_row *
                                    kExchangeWarpsPerBlock]);
          }
        }
        if (++head_slot == kExchangePipelineDepth)
          head_slot = 0;
        if (++tail_slot == kExchangePipelineDepth)
          tail_slot = 0;
      }

      // The rolling loop ends with depth - 1 padding groups. Drain them so
      // the next block_input iteration starts with an empty cp.async queue.
      __pipeline_wait_prior(0);
      __syncwarp();
    }

    // Preserve the existing per-lane system fence and reference-release
    // ordering. In particular, lane zero must not release insertion slots
    // before mapped-host stores from the other lanes are globally visible.
