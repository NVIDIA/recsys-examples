  ...
  for optimizer_type in ${OPTIMIZER_TYPE[@]}; do  
    for include_optim in ${INCLUDE_OPTIM[@]}; do
      torchrun \
        ...
        --mode "dump" \
        --optim ${include_optim} \
        --save-path "debug_weight_${optimizer_type}_${num_gpus}_${include_optim}" \
...
for num_load_gpus in ${NUM_GPUS[@]}; do
  for num_dump_gpus in ${NUM_GPUS[@]}; do
    for optimizer_type in ${OPTIMIZER_TYPE[@]}; do  
      for include_optim in ${INCLUDE_OPTIM[@]}; do
        echo "num_load_gpus: $num_load_gpus, num_dump_gpus: $num_dump_gpus, optimizer_type: $optimizer_type, include_optim: $include_optim"
        torchrun \
          --nnodes 1 \
          ...
          --mode "load" \
          --optim ${include_optim} \
          --save-path "debug_weight_${optimizer_type}_${num_dump_gpus}_${include_optim}" \
          --num-embedding-collections $NUM_EMBEDDING_COLLECTIONS \
          --num-embeddings $NUM_EMBEDDINGS \
          --multi-hot-sizes $MULTI_HOT_SIZES \
          --embedding-dim 16 || exit 1
