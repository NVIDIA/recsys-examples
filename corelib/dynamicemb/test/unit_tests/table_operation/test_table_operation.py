...
def test_table_incremental_dump(
    ...
    keys = torch.randperm(bucket_capacity, device=device, dtype=torch.int64)
    score_args = [
        ScoreArg(name="score1", value=get_scores(score_policy, keys), is_return=True)
    ]
    insert_results = torch.empty(
    ...
    assert set(dumped_keys.tolist()) == set(keys.tolist())
    ...
    for i in range(0, num_iteration, dump_interval):
        ...
        interval_keys = torch.arange(
            offset,
            offset + dump_interval * batch_size,
            device=device,
            dtype=torch.int64,
        )
        interval_existed_in_table = torch.empty(
            dump_interval * batch_size, device=device, dtype=torch.bool
        ).fill_(True)
        ...
        if score_policy != ScorePolicy.ACCUMULATE:
            assert ((named_scores["score1"].to(device) - undumped_score) >= 0).all()
            assert keys.numel() == num_remain
            ascend_dumped_keys, _ = torch.sort(keys.to(device))
            ...
            assert torch.isin(
                interval_keys[interval_existed_in_table], keys.to(device)
            ).all()

        # check 4: using min_score to count will get table's size
        dumped_keys, _ = table.incremental_dump({"score1": 0})
        assert dumped_keys.numel() == table.size()

        # log
        load_factor = table.size() / table.capacity()
        print(
            f"Load factor={load_factor:.3f}, there are {interval_existed_in_table.sum()} keys existed in the table for the last interval {dump_interval*batch_size}"
        )
