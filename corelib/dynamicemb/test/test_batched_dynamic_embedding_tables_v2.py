...
class PyDictStorage(Storage[DynamicEmbTableOptions, BaseDynamicEmbeddingOptimizer]):
    ...
    def exchange(self, request: CacheExchangeRequest) -> CacheExchangeResult:
        ...
        if miss_positions.numel() > 0:
            ...
            if found_positions.numel() > 0:
                cache_rows = request.cache_indices[found_positions]
                if bool((cache_rows < 0).any().item()):
                    raise RuntimeError(
                        "PyDictStorage cannot expose addressable direct rows; "
                        "an external exchange backend must provide direct "
                        "storage metadata when cache placement is rolled back"
                    )
                store_to_flat(
                    cache_state,
                    cache_rows,
        ...
        return CacheExchangeResult(
            ...
            direct_storage_rows=torch.full_like(storage_scores, -1),
            direct_storage_slots=torch.full_like(storage_scores, -1),
            direct_storage_table_ptrs=cache_state.table_ptrs_dev,
        )
    ...
    def release_cache_exchange_refs(
        ...
        # This reference backend stages every hit into cache and therefore
        # never retains a storage-side reference.
        cache.decrement_counter(cache_slots, table_ids)
