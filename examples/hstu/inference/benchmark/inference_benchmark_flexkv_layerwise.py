...
def forward_with_kvcache_only_onboard_layerwise(
    ...
    """Run layerwise onboard while timing only the reused-prefix forward."""
    with torch.inference_mode():
        dense_module = model_predict.dense_module
        kvcache_mgr = dense_module.kvcache
...
def run_scenario_gpu_hit_only_onboard_layerwise(
    model_predict,
    history_len: int,
    num_candidates: int,
    max_seqlen: int,
...
def run_scenario_gpu_miss_host_hit_only_onboard_layerwise(
    model_predict,
    history_len: int,
    num_candidates: int,
    max_seqlen: int,
...
def run_scenario_gpu_cpu_miss_ssd_hit_only_onboard_layerwise(
    model_predict,
    history_len: int,
    num_candidates: int,
    max_seqlen: int,
...
if __name__ == "__main__":
    ...
    mode = "only_onboard_layerwise" if args.only_onboard else "layerwise"
    ...
    print(
        ...
        f"mode={mode}, "
        f"force_skip_offload={args.force_skip_offload}, "
    ...
    try:
        with torch.inference_mode():
            if "gpu_hit" in scenarios:
                if args.only_onboard:
                    run_scenario_gpu_hit_only_onboard_layerwise(
                        model_predict=model_predict,
                        history_len=history_len,
                        num_candidates=cfg.num_candidates,
                        max_seqlen=max_seqlen,
                        timed_iters=cfg.timed_iters,
                        skip_offload=args.force_skip_offload,
                    )
                else:
                    run_scenario_gpu_hit(
                        model_predict=model_predict,
            ...
            if "cpu_hit" in scenarios:
                if args.only_onboard:
                    run_scenario_gpu_miss_host_hit_only_onboard_layerwise(
                        model_predict=model_predict,
                        history_len=history_len,
                        num_candidates=cfg.num_candidates,
                        max_seqlen=max_seqlen,
            ...
            if "ssd_hit" in scenarios:
                if args.only_onboard:
                    run_scenario_gpu_cpu_miss_ssd_hit_only_onboard_layerwise(
                        model_predict=model_predict,
                        history_len=history_len,
                        num_candidates=cfg.num_candidates,
                        max_seqlen=max_seqlen,
