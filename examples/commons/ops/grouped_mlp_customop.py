...
def _launch_silu_mul_bwd(
    ...
    _silu_mul_backward_kernel[grid](grad_gate, grad_up, grad_output_1d, gate_1d, up_1d, x_size)
...
def silu_mul(gate: torch.Tensor, up: torch.Tensor) -> torch.Tensor:
    ...
    """
    torch._check(gate.shape == up.shape, lambda: f"Shape mismatch: {gate.shape} vs {up.shape}")
...
@torch.library.custom_op("grouped_mlp::silu_mul_backward", mutates_args=(), device_types="cuda")
...
def _(grad_output: torch.Tensor, gate: torch.Tensor, up: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
...
@torch.library.custom_op("grouped_mlp::strided_bmm", mutates_args=(), device_types="cuda")
def strided_bmm(
    ...
    output = torch.empty(batch_size, num_groups, output_dim, device=x.device, dtype=x.dtype)
    # x: (B, N, K) -> (N, B, K)
    # weight: (N, K, M)
    # out: (N, B, M) -> (B, N, M)
...
@torch.library.custom_op("grouped_mlp::strided_bmm_backward", mutates_args=(), device_types="cuda")
...
def _(grad_output: torch.Tensor, x: torch.Tensor, weight: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
...
# =============================================================================
# [3] nn.Module: GroupedMLP_CustomOp (Plan A using Custom Ops)
# =============================================================================

class GroupedMLP_CustomOp(nn.Module):
    """
    Grouped MLP using custom ops (Plan A: BMM-based).
...
def copy_weights_ref_to_customop(
    ...
    customop_model: GroupedMLP_CustomOp,
):
...
def check_torch_compile(customop_model: GroupedMLP_CustomOp, batch_size: int) -> bool:
...
def run_opcheck(customop_model: GroupedMLP_CustomOp):
    ...
    examples_silu = [
        [torch.randn(32, 64, device="cuda", dtype=torch.bfloat16),
         torch.randn(32, 64, device="cuda", dtype=torch.bfloat16)],
        [torch.randn(16, 12, 128, device="cuda", dtype=torch.bfloat16, requires_grad=True),
         torch.randn(16, 12, 128, device="cuda", dtype=torch.bfloat16, requires_grad=True)],
    ]
    ...
    examples_bmm = [
        ...
        [torch.randn(16, 12, 256, device="cuda", dtype=torch.bfloat16, requires_grad=True),
         torch.randn(12, 256, 512, device="cuda", dtype=torch.bfloat16, requires_grad=True)],
    ]
...
def benchmark_silu_mul_bandwidth(
    ...
    bwd_bytes = 5 * numel * bytes_per_elem  # read grad_out, gate, up; write grad_gate, grad_up
...
""")
    ...
    fwd_ok, fwd_diff = check_forward_correctness(ref_model, customop_model, batch_size, dtype)
    print(f"\nForward:  {'✓ PASS' if fwd_ok else '✗ FAIL'}  (max_diff: {fwd_diff:.2e})")
    
    bwd_ok, bwd_diff = check_backward_correctness(ref_model, customop_model, batch_size, dtype)
    ...
    print(f"{'CustomOp (BMM)':<30} {customop_fwd:<12.4f} {ref_fwd/customop_fwd:<10.2f}x")
    ...
    print(f"{'CustomOp (BMM)':<30} {customop_fwdbwd:<12.4f} {ref_fwdbwd/customop_fwdbwd:<10.2f}x")
