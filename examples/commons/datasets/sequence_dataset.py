...
class SequenceDataset(IterableDataset[HSTUBatch]):
    ...
    Args:
        ...
        max_history_seqlen (int): The maximum sequence length.
    ...
    def __init__(
        ...
        *,
        ...
        self._num_contextual_features = len(contextual_feature_names)
        # ensure all contextual features exist in the dataset
        for name in self._contextual_feature_names + [
            item_feature_name,
            action_feature_name,
        ]:
            if name not in self._seq_logs_frame.columns:
                raise ValueError(f"feature {name} not in the dataset")
        # the hstu batch layout is:
        # [[contextual_features], [history item features | candidate], [jagged history action features | optional candidate action]]
        # we assume each contextual feature is a single token.
        self._feature_to_max_seqlen: Dict[str, int] = {
            **{name: 1 for name in self._contextual_feature_names},
            item_feature_name: max_history_seqlen + max_num_candidates,
            action_feature_name: max_history_seqlen + max_num_candidates,
        }
    ...
    def __iter__(self) -> Iterator[HSTUBatch]:
        for i in range(len(self)):
            ...
            for sample_id in sample_ids:
                # data is one sequence
                # [[contextual_features], [item_features], [action_features]]
                ...
                raw_item_seq = load_seq(data[self._item_feature_name])
                # ensure we have at least one history.
                if self._max_num_candidates >= len(raw_item_seq):
                    raise ValueError(
                        f"max_num_candidates: {self._max_num_candidates} > len(raw_item_seq): {len(raw_item_seq)}, please check data or decrease max_num_candidates"
                    )
                raw_item_seq = maybe_truncate_seq(
                    raw_item_seq, self._max_history_seqlen + self._max_num_candidates
                )
                # history_seq = raw_item_seq[: -self._max_num_candidates]
                candidate_seq = raw_item_seq[-self._max_num_candidates :]

                item_features.extend(raw_item_seq)
                item_features_seqlen.append(len(raw_item_seq))

                raw_action_seq = load_seq(data[self._action_feature_name])
                if self._load_candidate_action and self._max_num_candidates >= len(
                    raw_action_seq
                ):
                    raise ValueError(
                        f"max_num_candidates: {self._max_num_candidates} > len(raw_action_seq): {len(raw_action_seq)}, please check data or decrease max_num_candidates"
                    )
                raw_action_seq = maybe_truncate_seq(
                    raw_action_seq, self._max_history_seqlen + self._max_num_candidates
                )
                history_action_seq = raw_action_seq[: -self._max_num_candidates]
                candidate_action_seq = (
                    raw_action_seq[-self._max_num_candidates :]
                    if self._load_candidate_action
                    else []
                )

                action_seq = history_action_seq + candidate_action_seq
                action_features.extend(action_seq)
                action_features_seqlen.append(len(action_seq))
                if self._max_num_candidates > 0:
                    num_candidate = min(self._max_num_candidates, len(candidate_seq))
                    num_candidates.append(num_candidate)

                if self._num_tasks > 0:
                    # candidate action is only used for ranking task.
                    label = (
                        raw_action_seq[-self._max_num_candidates :]
                        if self._max_num_candidates > 0
                        else action_seq
            ...
            # contextual features are always 1.
            contextual_features_lengths_tensor = pad_tensor(
                self._batch_size - actual_batch_size,
                torch.full(
                    (actual_batch_size,), 1, device=self._device, dtype=torch.int64
                ),
            )

            item_features_tensor = torch.tensor(
                item_features, device=self._device, dtype=torch.int64
            )
            item_features_lengths_tensor = pad_tensor(
                self._batch_size - actual_batch_size,
                torch.tensor(
                    item_features_seqlen, device=self._device, dtype=torch.int64
                ),
            )
            action_features_tensor = torch.tensor(
                action_features, device=self._device, dtype=torch.int64
            )
            action_features_lengths_tensor = pad_tensor(
                self._batch_size - actual_batch_size,
                torch.tensor(
                    action_features_seqlen, device=self._device, dtype=torch.int64
                ),
            )
            num_candidates_tensor = pad_tensor(
                self._batch_size - actual_batch_size,
                torch.tensor(num_candidates, device=self._device, dtype=torch.int64),
            )
            features = KeyedJaggedTensor.from_lengths_sync(
                ...
                lengths=torch.concat(
                    [
                        contextual_features_lengths_tensor.repeat(
                            self._num_contextual_features
                        ),
                        item_features_lengths_tensor,
                        action_features_lengths_tensor,
                    ]
                ).long(),
            )
...
def get_dataset(
    ...
    *,
    ...
    Args:
        ...
        max_history_seqlen (int): The maximum length of sequences in the dataset.
